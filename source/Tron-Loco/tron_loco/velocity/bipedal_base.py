...
class ObservationsCfg:
    ...
    class ObsGroupCfg(ObservationGroupCfg):
        ...
        base_ang_vel = ObservationTermCfg(func=mdp.base_ang_vel, noise=Unoise(n_min=-0.2, n_max=0.2), history_length=10//10)
    ...
    policy: ObsGroupCfg = ObsGroupCfg()
...
class RewardsCfg:
    ...
    # joint_accel = RewardTermCfg(func=mdp.joint_acc_l2, weight=-2.5e-7*0.)
    joint_torque = RewardTermCfg(func=mdp.joint_torques_l2, weight=-1e-5)
    # joint_limits = RewardTermCfg(func=mdp.joint_pos_limits, weight=-1.0 *0.)
